  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      ...
      case "bodyChange":
        ...
        return;
      case "submitRequest":
        if (!draft.title.hasError && !draft.title.hasError) {
          draft.sendCount++;
        }
        return;
      ...
      try {
        ...
        if (response.data) {
          ...
          if (appState.user.username != response.data.author.username) {
            appDisptach({ type: "flashMessage", value: "You do not have permission to edit this post" });
            props.history.push("/");
          }
        } else {
    ...
    if (state.sendCount) {
      ...
      async function fetchPost() {
        try {
          ...
          appDisptach({ type: "flashMessage", value: "Post was updated." });
        } catch (e) {
          console.log("There was a problem.");
        }
      }
